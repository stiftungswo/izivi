...
# check backend (api)
composer global require "squizlabs/php_codesniffer=*" $COMPOSER_FLAGS
$HOME/.composer/vendor/bin/phpcbf --standard=psr2 api/ --ignore=vendor | grep "PHPCBF CAN FIX" > /dev/null
if [ $? -eq 0  ]; then
