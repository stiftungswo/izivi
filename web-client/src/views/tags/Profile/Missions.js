...
export default class Missions extends Component {
    ...
    renderMissions = (self, mission, isAdmin) => {
        ...
        if(mission!=null && mission.draft==null && isAdmin){
            aufgebotErhaltenButton.push(<button class="btn btn-primary" type="button" onClick={()=>{this.setReceivedDraft(self, missionKey)}}>Aufgebot erhalten</button>);
        }
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <form class="form-horizontal" action="javascript:;" onsubmit={()=>{this.saveMission(self, missionKey)}}>
                            <div class="form-group">
                                <label class="control-label col-sm-3" for={missionKey+'_specification'}>Pflichtenheft</label>
                                <div class="col-sm-9">
                                    <select value={""+self.state['result'][missionKey+'_specification']} id={missionKey+'_specification'} name={missionKey+'_specification'} class="form-control" onChange={(e)=>self.handleSelectChange(e)} required>
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-sm-9">
                                    <select value={""+self.state['result'][missionKey+'_mission_type']} id={missionKey+'_mission_type'} name={missionKey+'_mission_type'}  class="form-control" onChange={(e)=>self.handleSelectChange(e)}>
                            ...
                            <DatePicker value={self.state['result'][missionKey+'_probation_day']} id={missionKey+'_probation_day'} callback={self.handleDateChange} label="Datum" callbackOrigin={self} />
                            <div class="form-group">
                                <label class="control-label col-sm-3" for={missionKey+'_probation_day_comment'}>Bemerkungen zum Schnuppertag</label>
                                <div class="col-sm-9">
                                    <textarea rows="4" id={missionKey+'_probation_day_comment'} name={missionKey+'_probation_day_comment'} class="form-control" onChange={(e)=>self.handleTextareaChange(e)} value={self.state.result[missionKey+'_probation_day_comment']} />
                                </div>
                            </div>
                            ...
                            {mission == null || mission.draft==null ? <button class="btn btn-primary" type="submit">Daten speichern</button> : null}
                            &nbsp;
        ...
        if(self.state.result.missions!=null){
            ...
            for(var i=0; i<m.length; i++){
                ...
                if(m[i].end >= new Date() || m[i].feedback_done == 1){
                    disabledMission.push(<div class="col-xs-1"><a href="/user_feedback"><button class="btn btn-xs btn-info" disabled> Feedback</button></a></div>)
                } else {
                    disabledMission.push(<div class="col-xs-1"><a href="/user_feedback"><button class="btn btn-xs btn-info"> Feedback</button></a></div>)
                }
