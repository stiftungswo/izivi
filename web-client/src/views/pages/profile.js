...
export default class User extends Component {
    ...
    getMissionDays(missionKey){
        ...
        axios.get(
            ApiService.BASE_URL+'diensttage?start='+this.state.result[missionKey+'_start']+'&end='+this.state.result[missionKey+'_end'],
            { headers: { Authorization: "Bearer " + localStorage.getItem('jwtToken') } }
        ).then((response) => {
    ...
    getReportSheets() {
        ...
        axios.get(
            ApiService.BASE_URL + 'reportsheet/user/' + apiRoute,
            { headers: { Authorization: "Bearer " + localStorage.getItem('jwtToken') } }
        ).then((response) => {
    ...
    setReceivedDraft(missionKey){
        ...
        this.setState({loading:true, error:null});
        axios.post(
            ApiService.BASE_URL+'mission/'+missionKey+'/receivedDraft',
            null,
            { headers: { Authorization: "Bearer " + localStorage.getItem('jwtToken') } }
        ).then((response) => {
            this.getUser();
    ...
    getEditModal(mission, isAdmin){
        ...
        if(mission!=null && mission.draft==null && isAdmin){
            aufgebotErhaltenButton.push(<button class="btn btn-primary" type="button" onClick={()=>{this.setReceivedDraft(missionKey)}}>Aufgebot erhalten</button>);
        }

        return (<div id={'einsatzModal'+(mission!=null ? mission.id : '')} class="modal fade" role="dialog">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <button id="einsatzModalClose" type="button" class="close" data-dismiss="modal">&times;</button>
                        <h4 class="modal-title">Zivildiensteinsatz</h4>
                    </div>
                    <div class="modal-body">

                        <form class="form-horizontal" action="javascript:;" onsubmit={()=>{this.saveMission(missionKey)}}>
                            <div class="form-group">
                                <label class="control-label col-sm-3" for={missionKey+'_specification'}>Pflichtenheft</label>
                                <div class="col-sm-9">
                                    <select value={""+this.state['result'][missionKey+'_specification']} id={missionKey+'_specification'} name={missionKey+'_specification'} class="form-control" onChange={(e)=>this.handleSelectChange(e)} required>
                                        {specification_options}
                                    </select>
                                </div>
                            </div>
                            <div class="form-group">
                                <label class="control-label col-sm-3" for="newmission_mission_type">Einsatzart</label>
                                <div class="col-sm-9">
                                    <select id="newmission_mission_type" name="newmission_mission_type" class="form-control" onChange={(e)=>this.handleSelectChange(e)}>
                            ...
                            <DatePicker value={this.state['result'][missionKey+'_start']} id={missionKey+'_start'} label="Einsatzbeginn" callback={(e,origin)=>{this.handleDateChange(e,origin);this.getMissionDays(missionKey)}} callbackOrigin={this} />
                            <DatePicker value={this.state['result'][missionKey+'_end']} id={missionKey+'_end'} label="Einsatzende"callback={(e,origin)=>{this.handleDateChange(e,origin);this.getMissionDays(missionKey)}} callbackOrigin={this} />
                            <InputFieldWithHelpText  value={this.state['result'][missionKey+'_days']} id={missionKey+'_days'} label="Tage" popoverText={howerText_Tage} self={this} disabled="true"/>

                            <InputCheckbox value={this.state['result'][missionKey+'_first_time']} id={missionKey+'_first_time'} label="Erster SWO Einsatz" self={this} />
                            <InputCheckbox value={this.state['result'][missionKey+'_long_mission']} id={missionKey+'_long_mission'} label="Langer Einsatz oder Teil davon" self={this} />
                            <InputCheckbox value={this.state['result'][missionKey+'_probation_period']} id={missionKey+'_probation_period'} label="Probeeinsatz" self={this} />

                            <hr/>
                            <h4>Schnuppertag</h4>
                            <p>Tragen Sie nachfolgend ein, ob Sie bei der SWO einen Schnuppertag geleistet haben. Dieser wird dem Einsatz allenfalls angerechnet.</p>
                            <DatePicker value={this.state[missionKey+'_trial_mission_date']} id={missionKey+'_trial_mission_date'} label="Datum" self={this} />
                            <div class="form-group">
                                <label class="control-label col-sm-3" for={missionKey+'_trial_mission_comment'}>Bemerkungen zum Schnuppertag</label>
                                <div class="col-sm-9">
                                    <textarea rows="4" value={this.state['result'][missionKey+'_trial_mission_comment']} id={missionKey+'_trial_mission_comment'} name={missionKey+'_trial_mission_comment'} class="form-control" onChange={(e)=>this.handleTextareaChange(e)}></textarea>
                                </div>
                            </div>
    ...
    getInternalNote(result) {
        return (
            <div class="form-group">
                ...
                <div class="col-sm-9">
                    <textarea rows="4" id="internal_note" name="internal_note" class="form-control"
                                onChange={(e) => this.handleTextareaChange(e)}>{result.internal_note}</textarea>
                </div>
            </div>
        );
    }
