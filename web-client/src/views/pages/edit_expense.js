        ...
        if(sheet!=null){
			...
			<form>
				...
				<table border="0" cellspacing="0" cellpadding="4" class="table">
					...
					<tr>
						<td><input type="checkbox" name="done" id="fidDone" defaultChecked={sheet.done} onchange={(e)=>(this.handleChange(e))}/><label for="fidDone"> Erledigt</label></td>
						<td>&nbsp;</td>
						<td>&nbsp;</td>
						<td>&nbsp;</td>
						<td>&nbsp;</td>
						<td align="right">
							<button type="button" name="showProfile" onClick={() => { this.router.push('/profile/' + ApiService.getUserId()) }}>Profil anzeigen</button>
							&nbsp;&nbsp;
							<button type="button" name="saveExpense" onClick={()=>{this.save()}}>Speichern</button>
						</td>
					</tr>
