    ...
    getUsers() {
        ...
        axios.get(
            ...
            for(let i=0;i<users.length;i++) {
                if(i % 2 == 1){
                    temp.push(<tr class="todd">
                        <td>{users[i].zdp}</td>
                        <td><a href="/profile">{users[i].first_name} {users[i].last_name}</a></td>
                        <td>{users[i].start}</td>
                        <td>{users[i].end}</td>
                        <td>  </td>
                        <td><button type="button">Löschen</button></td>
                        <td><a href="/profile">{users[i].role}</a></td>
                    </tr>);
                } else {
                    temp.push(<tr class="teven">
                        <td>{users[i].zdp}</td>
                        <td><a href="/profile">{users[i].first_name} {users[i].last_name}</a></td>
                        <td>{users[i].start}</td>
                        <td>{users[i].end}</td>
