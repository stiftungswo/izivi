...
export default class UserList extends Component {
    ...
    deleteUser(user){
        axios.delete(
            ApiService.BASE_URL+'user/'+user.id,
            { headers: { Authorization: "Bearer " + localStorage.getItem('jwtToken') } }
        ).then((response) => {
            this.getUsers();
        }).catch((error) => {
    ...
    render() {
        ...
        for(let i=0;i<users.length;i++) {
            ...
            temp.push(<tr class={even ? 'teven' : 'todd'}>
                <td>{users[i].zdp}</td>
                <td><a href={'/profile/'+users[i].id}>{users[i].first_name} {users[i].last_name}</a></td>
                <td>{users[i].start}</td>
                <td>{users[i].end}</td>
                <td>{users[i].role}</td>
                <td><button type="button" onClick={()=>{if(confirm('Möchten Sie '+users[i].first_name+' '+users[i].last_name+' wirklich löschen?')){ this.deleteUser(users[i]) }}}>Löschen</button></td>
            </tr>);
