...
export default class UserFeedbackOverview extends Component {
    ...
    getFeedbackAnswers() {
        ...
        if(this.props.params.feedback_id){
            ...
            request = ApiService.BASE_URL+'user/feedback?date_from='+this.dbDate(this.state.date_from) + '&date_to='+this.dbDate(this.state.date_to);
        }
    ...
    getType1RowContent(numberOfAnswers, answers, totalAnswers, colSize = 3) {
        ...
        for (var i = 1; i <= numberOfAnswers; i++) {
            ...
            if(numberOfAnswers == 2) {
                ...
                if(i == 2) {
                    cols.push(<div class={"col-xs-6"}></div>);
                }
            }
            else if(numberOfAnswers == 6) {
                answerClasses = ["", "progress-bar-info", "progress-bar-info", "progress-bar-info", "progress-bar-info", "progress-bar-info", "progress-bar-info"];
            }
            else {
            ...
            cols.push(
                <div class={"col-xs-"+colSize}>
                    <div class={"progress vertical progress-striped " + answerClasses[i]}>
                        <div class="progress-bar" style={"height: " + (100 - (answerPerc * 100)) + "%;"}>{Math.round((answerPerc * 100))}</div>
                    </div>
                </div>
            );
        }
    ...
    render() {
        ...
        for(var x=0; x < answers.length; x++){
            ...
            if (answers[x].type == TYPE_SINGLE_QUESTION || 
                ...
                if (answers[x].type == TYPE_SINGLE_QUESTION_6) {
                    ...
                    feedbacks.push(
                        <div class="row">
                            ...
                            <div class="col-xs-2">
                                <div class="row">
                                    { this.getType1RowContent(2, answersCleaned, totalAnswers) }
                                </div>
                            </div>
                            <div class="col-xs-1">
                    ...
                    feedbacks.push(
                        <div class="row">
                            ...
                            <div class="col-xs-2">
                                <div class="row">
                                    { this.getType1RowContent(4, answersCleaned, totalAnswers) }
                                </div>
                            </div>
                            <div class="col-xs-1">
                                <label>{answers[x].opt2}</label>
                ...
                feedbacks.push(
                    ...
                    <div class="row">
                        <div class="col-xs-8">
                            <label><h4>{answers[x].question}</h4></label>
                        </div>
                        <div class="col-xs-1">
                            <label>{answers[x].opt1}</label>
                        </div>
                    ...
                    <div class="row">
                        ...
                        <div class="col-xs-6">
                            <textarea class="form-control" rows="10" cols="50"
                                      value={answers[x] ? answers[x]['answers'] : null} name={x}/>
                        </div>
                    </div>
                    <br /><br />
                    </div>)
                    ...
                    <Card>
                        {this.props.params.feedback_id == null &&
                            <div class="container top">
                                <div class="row">
                                    <div class="col-sm-4">
                                        <DatePicker id="date_from" label="Von" value={this.state.date_from}
                                                    callback={this.handleDateChange} callbackOrigin={this}/>
                                    </div>
                                    <div class="col-sm-4">
                                        <DatePicker id="date_to" label="Bis" value={this.state.date_to}
                                                    callback={this.handleDateChange} callbackOrigin={this}/>
                                    </div>
                                    <div class="col-sm-4">
