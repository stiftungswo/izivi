...
export default class MissionOverview extends Component {
    ...
    getMissions() {
        ...
        axios.get(
            ApiService.BASE_URL+'missions/'+this.state.year,
            { headers: { Authorization: "Bearer " + localStorage.getItem('jwtToken') } }
        ).then((response) => {
            this.setState({
    ...
    render(){
        ...
        for(var i=0; i<missions.length; i++){
            ...
            cells.push(<td><div class="no-print">{missions[i].zdp}</div></td>)
            cells.push(<td class="einsatz-zivi-name" style="text-align:left; padding-left:8px !important;" nowrap><a href={'/profile/'+missions[i].userid}>{missions[i].first_name} {missions[i].last_name}</a></td>)
                    ...
                    <Card>
                        ...
                        <table class="table table-striped table-bordered table-no-padding">
                            <thead>
                                ...
                                <tr>
                                    <td colspan="3" style="text-align:left; padding-left:8px !important; font-weight:bold;" nowrap>Ø / Woche: {(averageCount/52).toFixed(2)}</td>
                                    {averageHeaders}
                                </tr>
                            </thead>
                            <tbody>
