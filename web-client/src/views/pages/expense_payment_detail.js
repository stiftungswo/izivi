...
export default class ExpensePaymentDetail extends Component {
    ...
    setSheetState = (sheetId, state) => this.setState(update(this.state, {payment: {sheets: {
    ...
    getReportSheets() {
        ...
        axios.get(
            ApiService.BASE_URL+'reportsheet/payments/'+this.props.params.payment_id,
            { headers: { Authorization: "Bearer " + localStorage.getItem('jwtToken') } }
        ).then((response) => {
            this.setState({
		...
		return (
		    <Header>
                <div className="page page__expense">
                    <ScrollableCard>
                        <h2>Auszahlung vom {moment.tz(this.state.payment.created_at, "UTC").tz("Europe/Zurich").format('DD.MM.YYYY H:mm')}</h2>
                        <div>
                            <table class="table table-hover">
                                ...
                                <tbody>
                                  { sheets.map((sheet)=>(
                                  	<tr>
																			<td>{sheet.zdp}</td>
																			<td><a href={'/profile/'+sheet.userid}>{sheet.first_name} {sheet.last_name}</a></td>
																			<td>{sheet.iban}</td>
																			<td><a href={'/expense/'+sheet.report_sheet}>{'CHF '+this.formatRappen(sheet.amount/100)}</a></td>
																			<td>
																				{this.Confirmer(sheet)}
																			</td>
																		</tr>
																	))}
                                </tbody>
                            ...
                            <a href={ApiService.BASE_URL+'reportsheet/payments/xml/'+this.state.payment.id+'?jwttoken='+encodeURI(localStorage.getItem('jwtToken'))} class="btn btn-primary">zahlung.xml erneut herunterladen</a>
